...
export class ApiService {
  ...
  static getUserProfile(): UserProfile {
    ...
    return {
      ...
      bio: userObj.bio || '',
      stats: {
        total_managed_projects: userObj.stats?.total_managed_projects ?? 0,
        total_following_projects: userObj.stats?.total_following_projects ?? 0,
        total_budget_managed: userObj.stats?.total_budget_managed ?? 0,
        total_units_completed: userObj.stats?.total_units_completed ?? 0,
      },
      settings: {
        email_notifications: userObj.settings?.email_notifications ?? true,
        site_updates_push: userObj.settings?.site_updates_push ?? true,
        dark_mode: userObj.settings?.dark_mode ?? true,
        public_profile: userObj.settings?.public_profile ?? true,
      },
    };
  }
