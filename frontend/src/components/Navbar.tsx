...
import { Building2, Eye, User, Sparkles, Plus, LayoutGrid, LogOut } from 'lucide-react';
  ...
  return (
    <header className="bg-slate-950/90 backdrop-blur-2xl border-b border-slate-800/80 sticky top-0 z-50 px-4 lg:px-8 py-3 shadow-2xl">
      <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-3 md:gap-4">
        ...
        {/* Header Right Actions: Profile & Guest Mode */}
        <div className="flex items-center gap-2.5 justify-end">
          {isGuest ? (
            <div className="flex items-center gap-2">
              <span className="text-[11px] font-bold bg-cyan-500/20 text-cyan-300 px-3 py-1.5 rounded-2xl border border-cyan-500/30 flex items-center gap-1.5">
                <Eye className="w-3.5 h-3.5 text-cyan-400" />
                Misafir Modu (Salt Okunur)
              </span>
              {onOpenAuthModal && (
                <button
                  onClick={onOpenAuthModal}
                  className="bg-gradient-to-r from-amber-500 to-yellow-400 hover:from-amber-400 hover:to-yellow-300 text-slate-950 px-3.5 py-1.5 rounded-2xl text-xs font-bold transition cursor-pointer shadow-md shadow-amber-500/20"
                >
                  Giriş Yap / Kayıt Ol
                </button>
              )}
            ...
            <>
              ...
              >
                ...
                <span>{userProfile?.name || 'Kullanıcı'}</span>
              </button>

              {onLogout && (
                <button
                  onClick={onLogout}
                  className="flex items-center gap-1.5 bg-rose-500/10 hover:bg-rose-500/20 border border-rose-500/20 hover:border-rose-500/40 text-rose-400 px-3 py-2 rounded-2xl text-xs font-semibold transition cursor-pointer"
                  title="Oturumu Kapayarak Çıkış Yap"
                >
